...
class AttributeRulesTest {
  ...
  void colorTempKelvinFromMireds_produces_kelvin_caps() {
    ...
    assertThat(desc.getUnit()).isEqualTo("K");
    // raw conversion: minK = 1e6 / maxMireds = 2000, maxK = 1e6 / minMireds ≈ 6535.95
    assertThat(desc.getCaps().getMin()).isCloseTo(2000.0, within(0.1));
    assertThat(desc.getCaps().getMax()).isCloseTo(6535.95, within(0.5));
    assertThat(desc.getCaps().getStep()).isEqualTo(50.0);
  }
