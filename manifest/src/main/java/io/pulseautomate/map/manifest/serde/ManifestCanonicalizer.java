...
public final class ManifestCanonicalizer {
    ...
    public static Manifest canonicalize(Manifest m) {
        var entities = m.entities().stream()
                .sorted(Comparator.comparing(Entity::stable_id, Comparator.nullsLast(String::compareTo)))
                .map(ManifestCanonicalizer::canonEntity)
                .toList();

        var services = m.services().stream()
    ...
    private static Entity canonEntity(Entity e) {
        ...
        if (e.attributes() != null)
            attrs = e.attributes().entrySet().stream()
    ...
    private static Service canonService(Service s) {
        ...
        if (s.fields() != null)
            fields = s.fields().entrySet().stream()
                    .sorted(Map.Entry.comparingByKey())
                    .collect(Collectors.toMap(
                            Map.Entry::getKey,
                            Map.Entry::getValue,
