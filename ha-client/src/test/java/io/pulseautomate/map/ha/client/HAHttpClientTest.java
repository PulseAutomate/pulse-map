...
class HAHttpClientTest {
    ...
    void fetchStates_ok() throws Exception {
        try (MockWebServer server = new MockWebServer()) {
            server.enqueue(new MockResponse().setResponseCode(200)
                    .setBody(readResource("fixtures/states.json")));
            server.start();
            var cfg = new HAConfig(URI.create(server.url("/").toString()), "TOKEN",
                    java.time.Duration.ofSeconds(2), 0);
    ...
    void fetchServices_ok() throws Exception {
        try (MockWebServer server = new MockWebServer()) {
            server.enqueue(new MockResponse().setResponseCode(200)
                    .setBody(readResource("fixtures/services.json")));
            server.start();
            var cfg = new HAConfig(URI.create(server.url("/").toString()), "TOKEN",
                    java.time.Duration.ofSeconds(2), 0);
            var client = new HAHttpClient(cfg);
    ...
    void retriesOn500_thenSucceeds() throws Exception {
        try (MockWebServer server = new MockWebServer()) {
            ...
            server.start();
            var cfg = new HAConfig(URI.create(server.url("/").toString()), "TOKEN",
                    java.time.Duration.ofMillis(800), 1);
            var client = new HAHttpClient(cfg);
            var states = client.fetchStates();
            assertThat(states).hasSize(1);
        }
    ...
    void authFailure401_noRetry() throws Exception {
        try (MockWebServer server = new MockWebServer()) {
            ...
            server.start();
            var cfg = new HAConfig(URI.create(server.url("/").toString()), "BAD",
                    java.time.Duration.ofSeconds(1), 2);
            var client = new HAHttpClient(cfg);
